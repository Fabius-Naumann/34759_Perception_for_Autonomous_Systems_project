...
def init_kalman_state(cx, cy, depth=0.0):
    ...
    # Tighter initial covariance to avoid "flying" predictions
    P = np.eye(6, dtype=np.float32) * 40.0
    P[3, 3] = 20.0   # vx
    P[4, 4] = 20.0   # vy
    P[5, 5] = 10.0   # vz
...
class TrackManager:
    def __init__(
        ...
        # Tighter depth threshold and smoother weighting
        self.depth_thresh = 1.5
        self.lambda_depth = 5.0

        # Color pool for consistent visualization
        self.COLOR_POOL = [
            (255, 0, 0),
            (0, 255, 0),
            (0, 0, 255),
    ...
    # ---------------------------------------------------------
    #          Helper: distance gating (no speed expansion)
    # ---------------------------------------------------------
    def _allowed_distance(self, tr):
        """
        Gating distance in image plane.

        For YOLO detections, we keep it fixed, since velocity
        is not very reliable and using it to expand the gate
        tends to create "jumping" tracks.
        """
    ...
    def associate(self, detections, H, R):
        ...
        and Hungarian algorithm. Cost uses:
            ...
            - IoU gating between predicted bbox and detection bbox
        """
        assigned_tracks = set()
        assigned_dets = set()
        ...
        for t_idx, tr in enumerate(self.tracks):
            ...
            for d_idx, det in enumerate(detections):
                ...
                # IoU gating to avoid track jumping
                x2, y2, w2, h2 = det["x"], det["y"], det["w"], det["h"]
                ...
                # If bounding boxes barely overlap, it is probably a different person
                if iou < 0.05:
                    continue
                ...
                # Final cost: pixels plus weighted depth
    ...
    # ---------------------------------------------------------
    #      ASSOCIATE detections to PENDING tracks
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    #      PROMOTE confirmed pending tracks to real tracks
    # ---------------------------------------------------------
    def promote_tracks(self):
        ...
        for tr in confirmed:
            ...
            color = self.COLOR_POOL[tid % len(self.COLOR_POOL)]
    ...
    def remove_stale(self):
        # Confirmed tracks: respect max_invisible
        self.tracks = [
            tr for tr in self.tracks if tr["invisible"] < self.max_invisible
        ]
        # Pending tracks disappear quickly
        self.pending_tracks = [
