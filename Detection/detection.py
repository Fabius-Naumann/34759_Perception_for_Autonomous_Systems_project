...
cap = cv2.VideoCapture('Sequence 1.mp4')
ret, frame1 = cap.read()
if not ret:
...
tracks = []  # list of dicts: {id, x, P, bbox, invisible, age}
next_track_id = 0
...
while ret and i < 200:
    ...
    # Movement mask
    movement_mask = np.linalg.norm(flow, axis=2) > 3.0
    ...
    if len(tracks) > 0 and len(detections) > 0:
        ...
        while True:
            ...
            # Assign detection d_idx to track t_idx
            ...
            # Invalidate this row and column so they are not chosen again
